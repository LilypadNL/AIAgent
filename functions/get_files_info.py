...
def get_files_info(working_directory: str, directory: str = ".") -> str:
    ...
    try: 
        ...
        #Create the target directory from input
        working_dir_abs = os.path.abspath(working_directory)
        tar_dir = os.path.normpath(os.path.join(working_dir_abs, directory))

        #check to see if tar_dir is in working directory and that the working directory exists
        ...
        for file in os.listdir(tar_dir):
            isdir = os.path.isdir(os.path.join(tar_dir, file)) #note .isdir() and .getsize() take absolute path to the file
